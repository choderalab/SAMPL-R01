...
If synthesizing and testing each compound takes several days, a sufficiently reliable version of this workflow compresses roughly a year's work into a few days.
...
Applications projects tend not to take advantage of the latest methods---in part because their utility is not yet clear. 
...
A series of community blind prediction Challenges will test the limits of predictive techniques, providing a bridge between challenging but tractable problems and pharmacologically relevant but currently intractable problems.
...
First, we are able to ensure high data quality and build the capability to collect additional data for the same systems.
...
While the Drug Design Data Resource (D3R~\cite{Gathiaka:2016:JComputAidedMolDes}) also fields blind community Challenges, these rely on contributed experimental protein-ligand datasets from now-defunct pharmaceutical discovery programs~\cite{Gathiaka:2016:JComputAidedMolDes}; they provide a necessary \emph{benchmark} for real-world performance, but provide only modest new insight to drive improvement because a multitude of issues could be responsible for poor performance~\cite{D3R_lessons_learned}.
...
We focus on developing Challenges designed to drive the development and evaluation of solutions to specific \emph{component} problems that make it difficult to predict biomolecular interactions in a manner that allows the entire community to learn from both failure and success.
...
SAMPL Challenges focus on predicting properties that isolate likely sources of error, such as physical properties of small molecules (hydration free energies, aqueous tautomer ratios, partition or distribution coefficients between aqueous and nonpolar phases) or binding in systems of reduced complexity (such as host-guest complexation, or binding of fragments to trypsin and HIV integrase).
...
Distribution coefficients---the equilibrium ratio of solute concentration between aqueous and nonpolar phases at a given pH---mimic transfer free energies from aqueous to protein- or membrane-like environments, capturing many of the same physical characteristics but without associated challenges of receptor conformational sampling and specific ligand-receptor interactions.
...
Previous physical property SAMPL Challenges have already driven significant progress, but a frequent request is for modestly larger datasets and follow-up experiments to revisit specific measurements using orthogonal techniques~\cite{Mobley:2017:eScholarship}. 
While impossible with donated data, here we will expand the size and quality of Challenge datasets, allowing us to expand to provide greater coverage of the full dynamic range.%, which was not possible in SAMPL5~\cite{rustenburg_measuring_2016, Bannan:2016:JComputAidedMolDes}. 
...
{\bf Physical property data will be synergistically collected with binding datasets.}
%Solution-phase data and Challenges will also improve lessons learned from our binding Challenges.
Ligands studied in host-guest and protein-ligand binding Challenges will be prioritized for inclusion in solution-phase physical property datasets; if physical property data is already public for these, analogs or fragments will be included. 
For example, if a binding free energy proves difficult to predict, participants can determine if its solution-phase properties (such as protonation states or tautomers) were similarly difficult, thereby narrowing down the source of the error.
...
While D3R~\cite{Gathiaka:2016:JComputAidedMolDes} evaluates the accuracy of current protein-ligand binding methods, performance in each Challenge iteration varies widely depending on the difficulty of the donated pharmaceutical data and nature of the dataset.
...
For example, while kinases are targets of great interest to drug discovery, blind Challenges involving kinase targets also conflate issues of slow protein conformational dynamics~\cite{Lin:2013:Proc.Natl.Acad.Sci.}, protonation state effects ofor protein~\cite{Shan:2009:PNAS} and ligand~\cite{Szakacs:2005:JournalofMedicinalChemistry,Grante:2014:SpectrochimicaActaPartA:MolecularandBiomolecularSpectroscopy}, charged ligands, and other challenges;
% along with the standard challenges of conformational sampling and forcefield accuracy.
failures on such targets are thus often unexplained. 
...
If the D3R effort is not renewed, we will run SAMPL workshops independently, controlling costs via the model we use for our Workshops on Free Energy Methods in Drug Design---specifically, most participants will pay their own way to the workshop, seeking pharmaceutical and software industry sponsorships to defray costs. 
